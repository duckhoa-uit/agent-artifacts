...
export async function createApiKey(env: AppEnv, input: { owner: string; scopes: Scope[]; expiresAt?: number | null }): Promise<CreatedApiKey> {
...
export async function getArtifact(env: AppEnv, artifactId: string): Promise<ArtifactRow | null> {
  return env.DB.prepare("SELECT * FROM artifacts WHERE id = ?1 AND deleted_at IS NULL AND (expires_at IS NULL OR expires_at > ?2)")
...
export async function createShare(
  env: AppEnv,
  artifactId: string,
  creator: { apiKeyId?: string; actor?: string },
  ...
  ).bind(share.id, artifactId, await sha256(token), creator.apiKeyId ?? null, creator.actor ?? null, now(), expiresAt).run();
  return share;
}
